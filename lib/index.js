...
class Basura {
  ...
  _randBytes(num, reason = 'unspecified') {
    let buf = null
    if (this.opts.playback.length) {
      buf = this.opts.playback.shift()
      ...
      if (typeof buf === 'string') {
        ...
        if (buf.length !== num) {
          throw new Error(
            `Expected ${num} bytes, got ${buf.length}.  "${reason}"`
          )
        }
      }
    } else {
      buf = this.opts.randBytes(num)
    }
    if (this.opts.record) {
      this.record.push([buf, `${reason}:${num}`])
    }
    return buf
  }
